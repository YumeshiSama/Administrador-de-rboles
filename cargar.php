		...
		if ((!file_exists($targetFilePathCopa)) && (!file_exists($targetFilePathTronco))) {
			if((in_array($fileTypeCopa, $allowTypes)) && (in_array($fileTypeTronco, $allowTypes))){
				if((move_uploaded_file($_FILES["copa"]["tmp_name"], $targetFilePathCopa)) && (move_uploaded_file($_FILES["tronco"]["tmp_name"], $targetFilePathTronco))){		
					$sql = "INSERT INTO `arboles` (`id`, `id_usuario`, `id_edad`, `id_magnitud`, `extraible`, `poda`, `peligro_de_caida`, `id_copa`, `id_salud`, `id_localidad`, `id_especie`, `latitud`, `longitud`, `fecha_carga`, `foto_tronco`, `foto_copa`, `distancia_prox`, `comentario`) VALUES (NULL, '$id_usuario', '$edad', '$magnitud', '$extraccion', '$poda', '$caida', '$copa', '$salud', '0', '$especie', '$latitud', '$longitud', '$date', 'uploads/$fileNameTronco', 'uploads/$fileNameCopa', '$distancia', '$comentario')";
